###### GAM for Alexandrium phenology with mgcv - unified 2 ###
## V. POCHIC
# 2024-05-02
...
Season_Alex <- Table_Alex_zeros %>%
  ...
  mutate(across(.cols = contains('Alexandrium'), .fns = ~ ifelse(. %% 100 != 0, 0, .))) %>%
  # And creating some count variables for Dinophysis as a genus
  mutate(Alexandrium_genus = rowSums(across(contains('Alexandrium')))) %>%
  # create the log of abundance + 1
  mutate(log_c = log10(Alexandrium_genus+1)) %>%
  ...
  # distribution, contrary to Alexandrium_genus because the conversion from
  # 10 mL to 1L (*100) prevents some intermediate values (e.g., 150 cells.L-1)
  mutate(true_count = Alexandrium_genus/100) %>%
...
hist(Season_Alex$Year, breaks = 16)
...
hist(Season_Alex$Day, breaks = 26)
...
gam_Alex_newdata <- expand_grid(Day=seq(1, 365),
                                # We add a Year vector as it has become a factor
                                # of the model
                                Year=seq(min(Season_Alex$Year), 
                                        max(Season_Alex$Year)),
                                # We also add the site data
                                Code_point_Libelle = unique(Season_Alex$Code_point_Libelle))
...
ilink <- gam_Alex$family$linkinv

## Predict : add fit and se.fit on the **link** scale
gam_Alex_newdata <- bind_cols(gam_Alex_newdata, 
                              setNames(as_tibble(
                                predict(gam_Alex, gam_Alex_newdata, 
...
gam_Alex_newdata <- mutate(gam_Alex_newdata,
                           fit_resp  = ilink(fit_link),
                           right_upr = ilink(fit_link + (2 * se_link)),
                           right_lwr = ilink(fit_link - (2 * se_link)))
# Check the confidence interval. It should not extend below 0 (negative counts
# are impossible)
min(gam_Alex_newdata$right_lwr) # Nice :)
min(gam_Alex_newdata$fit_resp)
max(gam_Alex_newdata$right_upr) # Nice too
max(gam_Alex_newdata$fit_resp)
# But we can see that the confidence interval is ridiculously small around the
# min and max of the model fit.

# Plot

ggplot(gam_Alex_newdata, aes(x = Day, y = fit_resp))+
  geom_ribbon(aes(x=Day, ymin=right_lwr, ymax=right_upr), fill = 'grey70', alpha=0.7) +
  geom_line(linewidth = 1) +
  geom_point(data = Season_Alex, aes(x = Day, y = true_count, color = Year), shape = 21) +
...
### Checking the model
ModelOutputs<-data.frame(Fitted=fitted(gam_Alex),
                         Residuals=resid(gam_Alex))
...
Vb <- vcov(gam_Alex)
# New data
newd <- with(Season_Alex, data.frame(Day = rep(seq(1, 365, length = 365),
                                               length(unique(Year))*
...
# Add the Year vector
newd$Year <- rep(unique(Season_Alex$Year), 
                 365*length(unique(Season_Alex$Code_point_Libelle)))

newd <- group_by(newd, Year, Day) %>%
  arrange(Year, by_group = TRUE)

# And the site vector
newd$Code_point_Libelle <- rep(unique(Season_Alex$Code_point_Libelle), 
                               365*length(unique(Season_Alex$Year)))
...
pred <- predict(gam_Alex, newd, se.fit = TRUE, type = 'link')
...
Cg <- predict(gam_Alex, newd, type = "lpmatrix")
simDev <- Cg %*% t(BUdiff)
...
# write.csv2(response_pred_plot, 'response_pred_plot_Alex_20240502.csv',
...
# sims <- rmvn(N, mu = coef(gam_Alex), sig = Vb)
# fits <- Cg %*% t(sims)
# 
...
# And do the exact same thing in the Season_Alex dataset
Season_Alex <- Season_Alex %>%
  mutate(Code_point_Libelle = fct_relevel(Code_point_Libelle,
                                          'Antifer ponton pétrolier', 'Cabourg',
                                          'Men er Roue', 'Ouest Loscolo',
                                          'Le Cornard', 'Auger',
...
Season_Alex_crop <- Season_Alex %>%
  # Antifer
  filter(ifelse(true_count > 20 & Code_point_Libelle == 'Antifer ponton pétrolier',
                # if condition met, drop the line
                FALSE,
                # else, keep the line
                TRUE)) %>%
  # Cabourg
  filter(ifelse(true_count > 10 & Code_point_Libelle == 'Cabourg',
                # if condition met, drop the line
                FALSE,
                # else, keep the line
                TRUE)) %>%
  # Men er Roue
  filter(ifelse(true_count > 30 & Code_point_Libelle == 'Men er Roue',
                # if condition met, drop the line
                FALSE,
                # else, keep the line
                TRUE)) %>%
  # Ouest Loscolo
  filter(ifelse(true_count > 100 & Code_point_Libelle == 'Ouest Loscolo',
                # if condition met, drop the line
                FALSE,
                # else, keep the line
                TRUE)) %>%
  # Le Cornard
  filter(ifelse(true_count > 10 & Code_point_Libelle == 'Le Cornard',
                # if condition met, drop the line
                FALSE,
                # else, keep the line
                TRUE)) %>%
  ...
  filter(ifelse(true_count > 10 & Code_point_Libelle == 'Auger',
                ...
                TRUE)) %>%
  # Arcachon - Bouée 7
  filter(ifelse(true_count > 5 & Code_point_Libelle == 'Arcachon - Bouée 7',
                # if condition met, drop the line
                FALSE,
                # else, keep the line
                TRUE)) %>%
  ...
  filter(ifelse(true_count > 10 & Code_point_Libelle == 'Teychan bis',
                ...
                TRUE)) %>%
  # Parc Leucate
  filter(ifelse(true_count > 50 & Code_point_Libelle == 'Parc Leucate 2',
                # if condition met, drop the line
                FALSE,
                # else, keep the line
                TRUE)) %>%
  # Bouzigues
  filter(ifelse(true_count > 50 & Code_point_Libelle == 'Bouzigues (a)',
                # if condition met, drop the line
                FALSE,
                # else, keep the line
                TRUE)) %>%
  # Sète mer
  filter(ifelse(true_count > 10 & Code_point_Libelle == 'Sète mer',
                # if condition met, drop the line
                FALSE,
                # else, keep the line
                TRUE)) %>%
  # Diana
  filter(ifelse(true_count > 5 & Code_point_Libelle == 'Diana centre',
                # if condition met, drop the line
                FALSE,
                # else, keep the line
                TRUE))
...
ggplot(response_pred_plot, aes(x = Day, y = median.fit, 
  ...
  geom_point(data = Season_Alex_crop, aes(x = Day, y = true_count), 
             size = .8, alpha = .5) +
  labs(y = "Alexandrium cells observed",
       x = "Day of the year",
       title = "Poisson GAM of Alexandrium phenology" #,
  ...
  scale_x_continuous(limits = c(50,365)) +
  theme_classic()

# Saving plot
# ggsave('gam_Alex_allsites_crop2.tiff', dpi = 300, height = 175, width = 250,
#        units = 'mm', compression = 'lzw')
...
ggplot(pred_plot_cells_per_L, aes(x = Day, y = median.fit, 
  ...
  geom_path(lwd = 1) +
  # scale_y_continuous(limits = c(0,40)) +
  geom_point(data = Season_Alex_crop, aes(x = Day, y = Alexandrium_genus), 
             size = .8, alpha = .5) +
  labs(y = c(expression(paste("Alexandrium cells.L"^'-1'))),
       x = "Day of the year",
       title = "Poisson GAM of Alexandrium phenology" #,
       #subtitle = sprintf("Each line is one of %i draws from the Bayesian posterior distribution of the model", nrnd)
  ) +
...
Season_Alex_DayF_crop <- Season_Alex_crop %>%
  mutate(DayF = ifelse(# If Day = 50, DayF is set to 1
    Day == 50, 1,
    # Else, if Day > 50, DayF is set to Day - 49 (so Day 51 becomes Day 2 etc.)
    ifelse(Day > 50, Day - 49,
...
ggplot(response_pred_plot_DayF, aes(x = DayF, y = median.fit, 
  ...
  geom_line(lwd = 1) +
  # scale_y_continuous(limits = c(0,40)) +
  geom_point(data = Season_Alex_DayF_crop, aes(x = DayF, y = true_count), 
             size = .8, alpha = .5) +
  labs(y = "Alexandrium cells observed",
       x = "Day of the year",
       title = "Poisson GAM of Alexandrium phenology" #,
...
# Saving plot
# ggsave('gam_Alex_allsites_crop_DayF.tiff', dpi = 300, height = 270, width = 300,
#        units = 'mm', compression = 'lzw')
