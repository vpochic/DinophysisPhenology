...
qqplot_custom <- ggplot(qq_data_reordered) +
  ...
  theme_classic() +
  labs(title = 'qq-plot - Dinophysis GAM (2016-2022)',
...
RvFplot_custom <- ggplot(qq_data_reordered)+
  ...
  scale_color_discrete(type = pheno_palette4, guide = 'none') +
  theme_classic() +
  labs(title = 'RvF-plot - Dinophysis GAM (2016-2022)',
       y="Residuals",x="Fitted Values")

RvFplot_custom

# Save the plot
# ggsave('Plots/GAMs/Dinophysis/Rvfplot_custom_Dino_4sites.tiff', dpi = 300, height = 175, width = 250,
