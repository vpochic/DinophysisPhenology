...
Table_phyto_taxon <- read.csv2('Data/REPHY_outputs/Table1_phyto_taxon.csv', 
...
ggplot() +
  ...
  geom_line(data = gam_Dino_OL_newdata, aes(x = Date, y = fit_resp),
            ...
            color = '#435E7B') +
  # plot the data
  geom_point(data = subset(Season_Dino, Code_point_Libelle == 'Ouest Loscolo'), 
             aes(x = Date, y = true_count),
             color = '#11203E', size = 4,
             alpha = .3) +
...
# ggsave('Plots/GAMs/Dinophysis/GAM_OL_several_years.tiff', height = 200, width = 300,
#        dpi = 300, unit = 'mm', compression = 'lzw')
