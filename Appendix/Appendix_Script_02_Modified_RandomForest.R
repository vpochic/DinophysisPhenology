...
## Description: 
# In this script we bring subtle modifications to the random forest models
# presented in the paper, after some suggestions by Marc. The idea is to remove
# temperature from the predictors, because this variable integrates too much
# information. Also, instead of using u10 and v10 as a proxy for near-surface
# wind mixing, we will use a combination of both (u10 + v10)^2

# Otherwise, we'll closely follow the Random Forest script that we used in the
...
## Outputs:
## Files:
# Data/RF_outputs/Modified_RF/Randomforest_vip_data_11sites.csv
# Data/RF_outputs/Modified_RF/Randomforest_vip_data_4sites.csv
## --> Data tables with information on variable importance obtained after 20
## model runs (1 for each version of the model)

## Figures:
# Figures A03 and A04 - Evaluation of the performance of RF models (elements of
# the figures are generated, the full figures are assembled in ppt)
...
Table_data_RF <- left_join(Table_data_RF, Dino_GAM_derivative, 
                                     by = c('Code_point_Libelle', 'Date'), 
                                     suffix = c('','')) %>%
  # Compute a wind speed variables that integrates both u and v components
  # We take the absolute values of u10 and v10 because these variables can
  # be positive or negative, and we don't want them to cancel each other when
  # we add them.
  mutate(wind_speed = (abs(u10) + abs(v10))*(abs(u10) + abs(v10)))
...
Validation_data <- filter(Table_data_RF, 
  ...
  select(deriv, SALI, CHLOROA, Stratification_Index, 
         ssr, tcc, wind_speed) %>%
...
# Save the result!
write.csv2(dataplot_vip, 'Data/RF_outputs/Modified_RF/Randomforest_vip_data_11sites.csv',
           row.names = FALSE, fileEncoding = 'ISO-8859-1')

## Variable importance plot ####

# If necessary, import data previously saved (on variable importance)
dataplot_vip <- read.csv2('Data/RF_outputs/Modified_RF/Randomforest_vip_data_11sites.csv',
header = TRUE, fileEncoding = 'ISO-8859-1')
...
Table_data_RF_4sites <- left_join(Table_data_RF_4sites, Table_era5_daily_mean, 
                           by = c('Year', 'Day', 'Code_point_Libelle'),
                           suffix = c('','')) %>%
  # And create the wind speed integrated variable
  mutate(wind_speed = (abs(u10) + abs(v10))*(abs(u10) + abs(v10)))
...
# For the data table that we will use for the random forest, we only keep the
# response variable (deriv) and the predictor variables (SALI,
# CHLOROA, Stratification_Index, ssr, tcc, wind_speed, and others).
...
RF_data <- Table_data_RF_4sites %>%
  ...
  select(deriv, SALI, CHLOROA, Stratification_Index, 
         ssr, tcc, wind_speed, Mesodinium_genus, Allo,
...
# 'Data/RF_outputs/Modified_RF/Randomforest_vip_data_4sites.csv', header = TRUE,
# fileEncoding = 'ISO-8859-1')
