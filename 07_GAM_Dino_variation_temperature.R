...
## --> REPHY data to serve as template to project variation rate data onto
# Data/GAM_outputs/Dinophysis/All_sites/Dino_GAM_derivative.csv
## --> Dino variation rate data that is effectively the derivative of the GAM
# Data/REPHY_outputs/Table_hydro.csv
## --> The table with hydrology data from the REPHY, that contains temperature
## data we need
...
# We will only analyse the 8 Atlantic/Channel sampling sites where we have
# regular Dinophysis observations
Season_Dino_8sites <- filter(Season_Dino, Code_point_Libelle %in% 
                                c(# Baie de Seine
...
# Select only 8 sampling sites of interest
...
Table_data_multiyear <- left_join(Season_Dino_8sites, Dino_variation_rate, 
...
Table_data_multiyear <- left_join(Table_data_multiyear, Table_hydro_select, 
                                  ...
                                  suffix = c('','')) %>%
  # Get the sampling sites in the right order
...
# Color palette for 8 sampling sites
...
ggplot(gam_deriv_newdata, aes(x = TEMP, y = fit_resp, color = Code_point_Libelle)) +
  ...
  theme(strip.text.x = element_text(size = 7.2, colour = "black"),
        ...
        legend.key = element_rect(color = 'black', linewidth = .25),
