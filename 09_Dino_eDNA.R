...
## Description:
# Where is Dinophysis in winter? Let's look at eDNA to find out. This script
# quickly analyses eDNA data from a previous work by Roux et al. (2023) taken
# during 2 winter periods in South Brittany:
# (https://doi.org/10.12770/53dfe316-a6d4-426f-8983-56f2b37a5e8a)

## Files required:
# Data/eDNA/table_with_taxo_for_stats.tsv
# Data/eDNA/metadata_stats2.tsv
## --> tsv files with eDNA reads data and metadata
...
Table_ASV <- read_tsv('Data/eDNA/table_with_taxo_for_stats.tsv', col_names = TRUE)

# Table with metadata
Table_metadata <- read_tsv('Data/eDNA/metadata_stats2.tsv', col_names = TRUE)
...
#### Now let's look at that data (Plot) ####
...
ggplot(MetaB_Dino, aes(x = Date, y = Depth,
  ...
  # Modify the scale for size to show real values
  scale_size_continuous(breaks = c(5+exp(0.002), 5+exp(0.004), 5+exp(0.006),
                                   5+exp(0.008)),
                        labels = c('0.002', '0.004', '0.006', '0.008')) +
  # shape scale
  scale_shape_manual(values = shape_Dino) +
  # guides
  guides(shape = guide_legend('Dinophysis eDNA detection'),
         size = guide_legend('Fraction of Dinophysis reads')) +
  labs(y = 'Depth level') +
  # Theme
  theme_classic() +
